...
export class FormInputComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    this.missingInputText ??=
      this.options.missingInputText ?? defaultFormInputOptions.missingInputText;
    ...
    // this.questionPlacement ??=
    //   this.options.questionPlacement ?? defaultFormInputOptions.questionPlacement;
    ...
    this.validationMessages ??=
      this.options.validationMessages ?? defaultFormInputOptions.validationMessages;
    this.alignVertical ??= this.options.alignVertical ?? defaultFormInputOptions.alignVertical;
  }

  ngOnInit(): void {
    this.labelIsTemplate = typeof this.label != 'string';
  ...
  ngAfterViewInit() {
    if (this.multiInput) {
      this.inputElements =
        this.element.nativeElement.getElementsByClassName('multi-input')[0].children;
    }
    ...
    //       this.element.nativeElement.getElementsByClassName('input-group-append')[0];
    ...
    //       this.controlStatusSubscription = this.control.statusChanges.subscribe((status) => {
