...
export class FormGridComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    @Inject(FORM_GRID_OPTIONS) private options: FormGridOptions
  ) {
    ...
    this.errorHighlightOnTouched ??=
      this.options.errorHighlightOnTouched ?? defaultFormGridOptions.errorHighlightOnTouched;
  ...
  ngOnInit(): void {
    this.element.nativeElement.style.setProperty('--labelPadding', this.labelPadding);
    this.element.nativeElement.style.setProperty('--labelWidth', this.labelWidth);
    this.element.nativeElement.style.setProperty('--maxLabelWidth', this.maxLabelWidth);
    this.element.nativeElement.style.setProperty('--gridColumns', +this.columns);
  ...
  ngAfterViewInit() {
    const appendColonToComp = (comp: FormInputComponent | FormCustomComponent) => {
      if (
        !comp.label ||
        typeof comp.label != 'string' ||
        comp.label.replace(/\s/g, '').length == 0
      ) {
        return;
      }
